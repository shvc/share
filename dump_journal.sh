...
do
	...
	eval $(awk '/primary/{print "zone="$2};/sealedTime:/{print "sealedTime="$2}' $chunkid/${chunkid}.info)
	echo "zone=$zone, sealedTime=$sealedTime"
	# get chunk dtId
	...
	eval $(awk '/http/{print "dtId="$4}' $chunkid/${chunkid}.dtId)
